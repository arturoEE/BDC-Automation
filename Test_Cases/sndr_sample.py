...
##
def sndr_sample(FS_Set, CIC_Set, samplerate):
    ...
    awg1.configureChannel(1,'SQU',0.0,0.4,samplerate)
    ...
    LA.configureLogic()
    LA.setCaptureDuration(1/samplerate*(2**16))
    ...
    plt.show()
    ax.cla()
    result = [[float(item) for  item in DATA.synchronousDataTimeStamp], fftlib.convertCodeToVoltage(9,FS_Set, DATA.synchronousDataInt)]
    LA.closeCapture()
    LA.close()
    awg1.disableALL()
    smu1.disableALL()
