...
class spine(dft.Test):
    ...
    def configureInstruments(self):
        ...
        #self.smu1.enableALL()
        # Clock
        self.awg1.configureChannel(1,'SQU',0.0,0.8,1000)
    ...
    def run(self):
        ...
        for res in self.resistors:
            os.makedirs(os.path.join(generalresultsdir,str(res)), exist_ok=True)
            self.resultsdir = os.path.join(generalresultsdir,str(res))
            subresults = []
            for n in range(1):
                ...
                SNRvalues = []
                minMaxCodes = []
                #input("Press ENTER after Common Mode and PG are Set")
                ...
                self.awg1.enableALL()
                #CIC_Set = afs.autoFS()
                #CIC_Set = 0.682 AD2, 20
                #CIC_Set = 0.665# , AD4 20
                #CIC_Set = 0.681
                CIC_Set = 0.5
                #if CIC_Set > 0.77:
                #    CIC_Set = 0.77
                ...
                new_data_file = self.saveData(self.temploggingfolder,"tst"+str(n))
                # Post Process Measurement
                DATA = saleae_utils.SaleaeData(new_data_file+".csv", ["D0","D1","D2","D3","D4","D5","D6","D7","D8","D9","CLK", "D10"], self.trigger_channel,triggerType="FALLING")
                DATA.loadData()
                DATA.convertDataToHex()
                DATA.readHexAtTriggerEdges()
                DATA.convertSynchHexdataToInt()
                waveform_to_save = [[float(item) for  item in DATA.synchronousDataTimeStamp], [float(item) for item in DATA.synchronousDataInt]]
                ...
                mean, s1,s2,s3 = noiselib.findNoiseValueRemoveStrangeCodes(codes)
                ...
                print("SNR: " + str(SNR))
                print("Max Code:" + str(max(codes)))
                print("Min Code:" + str(min(codes)))
                print("-------------------------------------------------")
                fig, ax = plt.subplots()
                ax.plot(codes)
                plt.show()
                subresults.append(mean)
            print("Mean: " + str(np.mean(subresults)))
            input("Press ENTER once the resistance has been swapped")
